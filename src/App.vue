...
export default {
  ...
  methods: {
    checkAlarm() {
      ...
      setInterval(()=>{
        ...
        for (i=0; i < this.today_alarm_todos.length; i++) {
          console.log('i랑 today_alarm_todos완료여부', i, this.today_alarm_todos[i].completed)
          if(this.today_alarm_todos[i].alarm_hour === cur_hour && this.today_alarm_todos[i].alarm_min === cur_minute && this.today_alarm_todos[i].completed === "no"){
            // 지원 여부 확인 및 알람 띄우기
            if (!("Notification" in window)) {
              ...
              // db의 todo completed여부도 변경
              todoCompleted ({...this.today_alarm_todos[i], completed: "yes"},
              () => {
    ...
    getTodo : function() {
      ...
      let cur_hour = day.getHours();
      let cur_minute = day.getMinutes();
      let today = cur_year.toString() + cur_month.toString() + cur_day.toString();
      ...
      todoList (
        (res) => {
          ...
          for (i=0; i < this.today_todos.length; i++){
            if ((this.today_todos[i].completed === "no") && (this.today_todos[i].alarm_hour === cur_hour) && (this.today_todos[i].alarm_min < cur_minute)) {
              this.today_notAlarm_todos.push(this.today_todos[i]);
            } else if ((this.today_todos[i].completed === "no") && (this.today_todos[i].alarm_hour < cur_hour)) {
              this.today_notAlarm_todos.push(this.today_todos[i]);
            } else {
